...
const ChatBubble: React.FC<ChatBubbleProps> = ({ message, stopAnimationToken = 0 }) => {
    ...
    return (
        <motion.div
            ...
            className={clsx(
                "chat-bubble-row flex w-full mb-4",
                isBot ? "justify-start" : "justify-end"
            )}
        >
            <div className={clsx(
                "chat-bubble-wrap flex max-w-[92%] md:max-w-[78%]",
                isBot ? "flex-row" : "flex-row-reverse"
            )}>
                {/* Avatar */}
                <div className={clsx(
                    "chat-avatar flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center overflow-hidden border-2 shadow-lg",
                ...
                <div className={clsx("flex min-w-0 flex-col", isBot ? "items-start" : "items-end")}>
                    {/* Bubble */}
                    <div className={twMerge(
                        "chat-bubble relative px-5 py-3 rounded-2xl text-sm md:text-base shadow-md backdrop-blur-sm overflow-hidden",
                        isBot
